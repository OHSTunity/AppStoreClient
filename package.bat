...
REM Prepare Content
md "%~dp0temp\wwwroot"
xcopy "%~dp0src\AppStoreClient\wwwroot" "%~dp0temp\wwwroot" /s /e

REM Copy icon and config
xcopy "%~dp0src\AppStoreClient\package\*.png" "%~dp0temp"
xcopy "%~dp0src\AppStoreClient\package\*.config" "%~dp0temp"
